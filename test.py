...
def build_string_to_sign(api_key: str, params: dict, timestamp_ms: str) -> str:
    """构建待签名字符串: apiKey={apiKey}&timestamp={timestamp}&{排序后的参数}"""
...
def call_firo_api(path: str, params: dict = None, method: str = "GET") -> dict:
    ...
    string_to_sign = build_string_to_sign(API_KEY, params, timestamp_ms)
    signature = generate_signature(PRIVATE_KEY_PEM, string_to_sign)
...
def test_text_match_results_single():
    """4. 竞彩开奖结果 (单日查询，使用 startDate/endDate)"""
    return call_firo_api("/firo/text/match-results", {
...
def main():
    ...
    tests = [
        ...
        ("4. /firo/text/match-results (startDate=endDate)", test_text_match_results_single),
